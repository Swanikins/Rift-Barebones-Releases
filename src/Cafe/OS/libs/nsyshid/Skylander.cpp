...
namespace nsyshid
{
	...
	{
		const auto status = g_skyportal.GetStatus();
		const uint32 readLength = std::min<uint32>(message->length, static_cast<uint32>(status.size()));
		memcpy(message->data, status.data(), readLength);
		message->bytesRead = readLength;
	...
	void SkylanderUSB::ControlTransfer(uint8* buf, uint32 length)
	{
		...
		if (length == 0)
		{
			cemuLog_log(LogType::Force, "Skylanders portal: ignored empty control transfer");
			return;
		}

		auto requireLength = [length, command = buf[0]](uint32 required) {
			if (length >= required)
				return true;
			cemuLog_log(LogType::Force,
				"Skylanders portal: ignored truncated '{}' command ({} bytes, expected at least {})",
				static_cast<char>(command), length, required);
			return false;
		};
	...
	{
		std::unique_ptr<FileStream> skyFile(FileStream::createFile2(pathName));
		if (!skyFile)
		{
			return false;
		}
		...
		return skyFile->writeData(data.data(), static_cast<sint32>(data.size())) == data.size();
	}

	std::array<std::optional<std::pair<uint16, uint16>>, MAX_SKYLANDERS> SkylanderUSB::GetLoadedSkylanders()
	{
		std::lock_guard lock(m_skyMutex);
		std::array<std::optional<std::pair<uint16, uint16>>, MAX_SKYLANDERS> result{};
		for (size_t i = 0; i < m_skylanders.size(); ++i)
		{
			const auto& figure = m_skylanders[i];
			if (!(figure.status & 1)) continue;
			const uint16 id = figure.data[0x10] | (uint16(figure.data[0x11]) << 8);
			const uint16 variant = figure.data[0x1C] | (uint16(figure.data[0x1D]) << 8);
			result[i] = std::pair{id, variant};
		}
		return result;
	}

	std::optional<SkylanderUSB::FigureSnapshot> SkylanderUSB::GetSkylanderSnapshot(uint8 skyNum)
	{
		std::lock_guard lock(m_skyMutex);
		if (skyNum >= m_skylanders.size())
			return std::nullopt;
	...
	const std::map<const std::pair<const uint16, const uint16>, const char*>& SkylanderUSB::GetListSkylanders()
	{
