...
namespace nsyshid
{
	...
	struct RetiredHIDSlot
	{
		size_t index;
		std::chrono::steady_clock::time_point reusableAfter;
	};
	std::queue<RetiredHIDSlot> HIDRetiredIndexQueue;

	void ReclaimRetiredHIDSlots()
	{
		const auto now = std::chrono::steady_clock::now();
		while (!HIDRetiredIndexQueue.empty() && HIDRetiredIndexQueue.front().reusableAfter <= now)
		{
			HIDPoolIndexQueue.push(HIDRetiredIndexQueue.front().index);
			HIDRetiredIndexQueue.pop();
		}
	}
	...
	bool AttachDevice(const std::shared_ptr<Device>& device)
	{
		...
		cemuLog_log(LogType::Force, "nsyshid.AttachDevice(): device attached: {:04x}:{:04x}, handle {}",
						 device->m_vendorId,
						 device->m_productId,
						 static_cast<uint32>(device->m_hid->handle));
		return true;
	}
	...
	void DetachDevice(const std::shared_ptr<Device>& device)
	{
		...
		cemuLog_log(LogType::Force, "nsyshid.DetachDevice(): device removed: {:04x}:{:04x}",
						 device->m_vendorId,
						 device->m_productId);
	}
	...
	void _hidGetDescriptorAsync(std::shared_ptr<Device> device, uint32 hidHandle, uint8 descType, uint8 descIndex, uint16 lang, uint8* output, uint32 outputMaxLength, MPTR callbackFuncMPTR, MPTR callbackParamMPTR)
	{
	...
	void export_HIDGetDescriptor(PPCInterpreter_t* hCPU)
	{
		...
		{
			// asynchronous
			std::thread(&_hidGetDescriptorAsync, device, hidHandle, descType, descIndex, lang, output, outputMaxLength, cbFuncMPTR, cbParamMPTR)
				.detach();
			returnCode = 0;
		}
	...
	void _hidSetIdleAsync(std::shared_ptr<Device> device, uint32 hidHandle, uint8 ifIndex, uint8 reportId, uint8 duration, MPTR callbackFuncMPTR, MPTR callbackParamMPTR)
	{
	...
	void export_HIDSetIdle(PPCInterpreter_t* hCPU)
	{
		...
		{
			// asynchronous
			std::thread(&_hidSetIdleAsync, device, hidHandle, ifIndex, reportId, duration, callbackFuncMPTR, callbackParamMPTR)
				.detach();
			returnCode = 0;
		}
		osLib_returnFromFunction(hCPU, returnCode);
	...
	void _hidSetProtocolAsync(std::shared_ptr<Device> device, uint32 hidHandle, uint8 ifIndex, uint8 protocol, MPTR callbackFuncMPTR, MPTR callbackParamMPTR)
	{
	...
	void export_HIDSetProtocol(PPCInterpreter_t* hCPU)
	{
		...
		{
			// asynchronous
			std::thread(&_hidSetProtocolAsync, device, hidHandle, ifIndex, protocol, callbackFuncMPTR, callbackParamMPTR)
				.detach();
			returnCode = 0;
		}
		osLib_returnFromFunction(hCPU, returnCode);
	...
	void _hidSetReportAsync(std::shared_ptr<Device> device, uint32 hidHandle, uint8 reportType, uint8 reportId, uint8* data, uint32 length,
	...
	void export_HIDSetReport(PPCInterpreter_t* hCPU)
	{
		...
		{
			// asynchronous
			std::thread(&_hidSetReportAsync, device, hidHandle, reportType, reportId, data, dataLength,
	...
	void _hidReadAsync(std::shared_ptr<Device> device, uint32 hidHandle,
					   ...
					   MPTR callbackParamMPTR)
	{
		...
		DoHIDTransferCallback(callbackFuncMPTR, callbackParamMPTR, hidHandle, errorCode,
	...
	void export_HIDRead(PPCInterpreter_t* hCPU)
	{
		...
		if (callbackFuncMPTR != MPTR_NULL)
		{
			...
			std::thread(&_hidReadAsync, device, hidHandle, data, maxLength, callbackFuncMPTR, callbackParamMPTR).detach();
			returnCode = 0;
		}
		else
		{
	...
	void _hidWriteAsync(std::shared_ptr<Device> device, uint32 hidHandle,
						uint8* data,
						sint32 maxLength,
						MPTR callbackFuncMPTR,
						MPTR callbackParamMPTR)
	...
	void export_HIDWrite(PPCInterpreter_t* hCPU)
	{
		...
		{
			// asynchronous transfer
			std::thread(&_hidWriteAsync, device, hidHandle, data, maxLength, callbackFuncMPTR, callbackParamMPTR).detach();
			returnCode = 0;
		}
		else
		{
