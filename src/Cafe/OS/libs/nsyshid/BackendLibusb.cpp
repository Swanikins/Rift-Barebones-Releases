...
namespace nsyshid::backend::libusb
{
	...
	void BackendLibusb::RefreshSkylanderPortal()
	{
		...
		const ssize_t deviceCount = libusb_get_device_list(m_ctx, &devices);
		if (deviceCount < 0)
			return;

		for (int i = 0; devices[i] != nullptr; ++i)
			AttachSkylanderPortal(devices[i]);

		libusb_free_device_list(devices, 1);
	}

	void BackendLibusb::AttachSkylanderPortal(libusb_device* dev)
	{
		...
		if (device && !AttachDevice(device))
		{
			cemuLog_log(LogType::Force, "nsyshid::BackendLibusb: failed to attach Skylanders portal");
		}
	}
	...
	void DeviceLibusb::CloseDevice()
	{
		...
		if (IsOpened())
		{
			...
			if (configResult == LIBUSB_SUCCESS && configNum >= 0 &&
				static_cast<size_t>(configNum) < m_config_descriptors.size() && m_config_descriptors[configNum])
			{
				for (uint8 interfaceIndex = 0;
					 interfaceIndex < m_config_descriptors[configNum]->bNumInterfaces; ++interfaceIndex)
				{
					const int releaseResult = libusb_release_interface(handle, interfaceIndex);
					if (releaseResult < LIBUSB_SUCCESS && releaseResult != LIBUSB_ERROR_NO_DEVICE &&
						releaseResult != LIBUSB_ERROR_NOT_FOUND)
					{
						cemuLog_log(LogType::Force, "Failed to release interface {} for config {}: {}",
							interfaceIndex, configNum, libusb_error_name(releaseResult));
					}
				}
			}
			libusb_close(handle);
