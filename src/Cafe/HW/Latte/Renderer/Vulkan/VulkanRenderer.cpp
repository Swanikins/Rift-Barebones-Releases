...
ImTextureID VulkanRenderer::GenerateTextureRGBA(const std::vector<uint8>& data, const Vector2i& size)
{
	try
	{
		return (ImTextureID)ImGui_ImplVulkan_GenerateTexture(m_state.currentCommandBuffer, data, size);
	}
	catch (const std::exception& ex)
	{
		cemuLog_log(LogType::Force, "can't generate RGBA imgui texture: {}", ex.what());
		return nullptr;
	}
}
