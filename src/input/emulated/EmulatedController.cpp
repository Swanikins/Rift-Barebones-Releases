...
std::atomic_uint32_t EmulatedController::s_menuToggleRequests{0};
std::atomic_size_t EmulatedController::s_menuToggleOwner{0};
std::atomic_bool EmulatedController::s_riftInputCaptured{false};

void EmulatedController::SetRiftInputCaptured(bool captured)
{
	s_riftInputCaptured.store(captured, std::memory_order_release);
}

bool EmulatedController::IsRiftInputCaptured()
{
	return s_riftInputCaptured.load(std::memory_order_acquire);
}

uint32 EmulatedController::ConsumeMenuToggleRequests(size_t& playerIndex)
{
	const uint32 requests = s_menuToggleRequests.exchange(0, std::memory_order_acq_rel);
	if (requests)
		playerIndex = s_menuToggleOwner.load(std::memory_order_relaxed);
	return requests;
}
...
void EmulatedController::controllers_update_states()
{
	...
	const bool chordDown = is_quick_menu_down();
	const bool wasDown = m_quickMenuChordDown.exchange(chordDown, std::memory_order_acq_rel);
	if (chordDown && !wasDown)
	{
		SetRiftInputCaptured(true);
		s_menuToggleOwner.store(m_player_index, std::memory_order_relaxed);
		s_menuToggleRequests.fetch_add(1, std::memory_order_release);
	}
}
