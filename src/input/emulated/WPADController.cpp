...
void WPADController::WPADRead(WPADStatus_t* status)
{
	...
	const bool inputCaptured = IsRiftInputCaptured();
	uint32 button = 0;
	for (uint32 i = 1; !inputCaptured && i < get_highest_mapping_id(); ++i)
	{
	...
	case kDataFormat_FREESTYLE_ACC_DPD:
	{
		...
		auto axis = inputCaptured ? glm::vec2{} : get_axis();
		axis *= 127.0f;
	...
	case kDataFormat_TAIKO:
	{
		...
		auto rotation = inputCaptured ? glm::vec2{} : get_rotation();
		rotation *= 2048.0f;
...
void WPADController::KPADRead(KPADStatus_t& status, const BtnRepeat& repeat)
{
	...
	// axis
	const auto axis = inputCaptured ? glm::vec2{} : get_axis();
	const auto rotation = inputCaptured ? glm::vec2{} : get_rotation();
	...
	*trigger = ~m_last_holdvalue & *hold;
	...
	if (!inputCaptured && has_position() && visibility != PositionVisibility::NONE)
	{
	...
	case Wiimote:
		...
		if(!inputCaptured && has_second_motion())
		{
