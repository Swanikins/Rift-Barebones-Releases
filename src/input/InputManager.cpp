...
bool InputManager::load(size_t player_index, std::string_view filename)
{
	...
	auto backup_file = file_path;
	backup_file += ".bak";
	if (!fs::exists(file_path) && fs::exists(backup_file))
	{
		std::error_code recovery_error;
		fs::rename(backup_file, file_path, recovery_error);
		if (recovery_error)
...
bool InputManager::save(size_t player_index, std::string_view filename)
{
	...
	FileStream* output = FileStream::createFile2(temporary_path);
	if (!output)
		return false;
	std::stringstream xmlData;
	doc.save(xmlData);
	std::string xmlStr = xmlData.str();
	output->writeData(xmlStr.data(), xmlStr.size());
	delete output;

	std::error_code ec;
	fs::remove(backup_path, ec);
	ec.clear();
	const bool had_previous = fs::exists(file_path, ec);
	if (ec)
	{
	...
	fs::rename(temporary_path, file_path, ec);
	if (ec)
	{
		std::error_code restore_error;
		if (had_previous)
			fs::rename(backup_path, file_path, restore_error);
		fs::remove(temporary_path, restore_error);
		return false;
	}
	fs::remove(backup_path, ec);
	return true;
