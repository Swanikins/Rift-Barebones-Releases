...
bool SDLController::connect()
{
	...
	SDL_JoystickGUID resolved_guid = m_guid;
	size_t resolved_guid_index = m_guid_index;
	const auto index = m_provider->get_index(m_guid_index, m_guid, m_display_name,
		&resolved_guid, &resolved_guid_index);
	if (index < 0)
		return false;
	...
	if (!(resolved_guid == m_guid) || resolved_guid_index != m_guid_index)
	{
		m_guid = resolved_guid;
		m_guid_index = resolved_guid_index;
		char guid_string[64];
		SDL_JoystickGetGUIDString(m_guid, guid_string, std::size(guid_string));
		m_uuid = fmt::format("{}_{}", m_guid_index, guid_string);
	}
