...
int SDLControllerProvider::get_index(size_t guid_index, const SDL_JoystickGUID& guid,
	std::string_view display_name, SDL_JoystickGUID* resolved_guid, size_t* resolved_guid_index) const
{
	size_t index = 0;
		...
		if (SDL_JoystickGetDeviceType(i) == SDL_JOYSTICK_TYPE_GAMECONTROLLER)
		{
			if(guid == SDL_JoystickGetDeviceGUID(i))
			{
				if (index == guid_index)
				{
					if (resolved_guid)
						*resolved_guid = guid;
					if (resolved_guid_index)
						*resolved_guid_index = guid_index;
					return i;
				}
	...
	if (!display_name.empty())
	{
		...
		for (int i = 0; i < SDL_NumJoysticks(); ++i)
		{
			...
			const auto candidate_guid = SDL_JoystickGetDeviceGUID(i);
			const size_t candidate_guid_index = guid_counter[candidate_guid]++;
			if (name_index++ != guid_index)
				continue;

			if (resolved_guid)
				*resolved_guid = candidate_guid;
			if (resolved_guid_index)
				*resolved_guid_index = candidate_guid_index;
			return i;
		}
	}
