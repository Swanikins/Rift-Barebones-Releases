...
wxPanel* EmulatedUSBDeviceFrame::AddSkylanderPage(wxNotebook* notebook)
{
	...
	auto* collectionButton = new wxButton(box, wxID_ANY, _("Browse Skylander Collection"));
	collectionButton->SetToolTip(_("Choose a figure from the collection folder configured in General Settings."));
	collectionButton->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
		BrowseSkylanderCollection();
	});
	boxSizer->Add(collectionButton, 0, wxEXPAND | wxALL, 4);
...
void EmulatedUSBDeviceFrame::BrowseSkylanderCollection()
{
	const auto configuredPath = GetConfig().emulated_usb_devices.skylander_collection_path.GetValue();
	if (configuredPath.empty())
	{
		wxMessageBox(_("Choose a Skylander Collection folder in Options > General Settings first."),
			_("Skylander Collection"), wxOK | wxICON_INFORMATION, this);
		return;
	}

	const fs::path collectionPath = _utf8ToPath(configuredPath);
	if (!fs::is_directory(collectionPath))
	{
		wxMessageBox(_("The configured Skylander Collection folder could not be found."),
	...
	if (!catalog.Reload())
	{
		wxMessageBox(wxHelper::FromUtf8(catalog.GetError()), _("Skylander Collection"),
			wxOK | wxICON_ERROR, this);
		return;
	}
	const auto& figures = catalog.GetCollection();
	if (figures.empty())
	{
		wxMessageBox(_("No supported Skylander files were found in that folder."),
			_("Skylander Collection"), wxOK | wxICON_INFORMATION, this);
		return;
	}

	wxArrayString figureNames;
	for (const auto& figure : figures)
	{
		figureNames.Add(wxHelper::FromUtf8(fmt::format("{}  —  {}", figure.name,
			_pathToUtf8(figure.filePath.lexically_relative(collectionPath)))));
	}
	wxSingleChoiceDialog figureDialog(this, _("Choose a Skylander"), _("Skylander Collection"), figureNames);
	if (figureDialog.ShowModal() != wxID_OK)
		return;

	wxArrayString slots;
	for (uint8 i = 0; i < nsyshid::MAX_SKYLANDERS; ++i)
		slots.Add(wxString::Format(_("Portal Slot %u"), i + 1));
	wxSingleChoiceDialog slotDialog(this, _("Choose where to place this figure"), _("Virtual Portal"), slots);
	if (slotDialog.ShowModal() != wxID_OK)
		return;

	LoadSkylanderPath(static_cast<uint8>(slotDialog.GetSelection()),
		wxHelper::FromPath(figures[figureDialog.GetSelection()].filePath));
}
