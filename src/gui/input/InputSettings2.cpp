...
wxWindow* InputSettings2::initialize_page(size_t index)
{
	...
	{
		sizer->Add(new wxStaticText(page, wxID_ANY, _("Named profile"), wxDefaultPosition, wxDefaultSize, 0), wxGBPosition(0, 0), wxDefaultSpan, wxALIGN_CENTER_VERTICAL | wxALL, 5);
		...
		auto* save_bttn = new wxButton(page, wxID_ANY, _("Save copy"));
		save_bttn->Disable();
		...
		auto* profile_status = new wxStaticText(page, wxID_ANY, _("Player setup saves automatically. Named profiles are optional backups."), wxDefaultPosition, wxDefaultSize, 0);
		profile_status->SetMinSize(wxSize(220, -1));
		profile_status->Wrap(220);
		...
		{
			profile_status->SetLabelText(_("This controller is set by the game profile, so changes apply only to this game."));
			profile_status->Wrap(220);
	...
	{
		// emulated controller
		sizer->Add(new wxStaticText(page, wxID_ANY, _("Wii U controller")), wxGBPosition(2, 0), wxDefaultSpan, wxALIGN_CENTER_VERTICAL | wxALL, 5);
	...
	{
		// controller api
		sizer->Add(new wxStaticText(page, wxID_ANY, _("Input device")), wxGBPosition(4, 0), wxDefaultSpan, wxALIGN_CENTER_VERTICAL | wxALL, 5);
...

		{
			...
			auto* add_api = new wxButton(page, wxID_ANY, _("Add device..."), wxDefaultPosition, wxDefaultSize, wxBU_EXACTFIT);
			add_api->Bind(wxEVT_BUTTON, &InputSettings2::on_controller_add, this);
			bttn_sizer->Add(add_api, 0, wxALL, 5);

			auto* remove_api = new wxButton(page, wxID_ANY, _("Remove"), wxDefaultPosition, wxDefaultSize, wxBU_EXACTFIT);
		...
		auto* settings = new wxButton(page, wxID_ANY, _("Device settings"), wxDefaultPosition, wxDefaultSize, 0);
		...
		auto* clear = new wxButton(page, wxID_ANY, _("Clear mappings"), wxDefaultPosition, wxDefaultSize, 0);
...
void InputSettings2::update_state()
{
	...
	if (InputManager::instance().is_gameprofile_set(m_notebook->GetSelection()))
	{
		page_data.m_profile_status->SetLabelText(_("This controller is set by the game profile, so changes apply only to this game."));
		page_data.m_profile_status->SetForegroundColour(wxTheColourDatabase->Find("ERROR"));
	}
	else
	{
		page_data.m_profile_status->SetLabelText(_("Player setup saves automatically. Named profiles are optional backups."));
		page_data.m_profile_status->SetForegroundColour(wxNullColour);
	}
	page_data.m_profile_status->Wrap(220);
	page_data.m_profile_status->Show();
...
void InputSettings2::save_current_player()
{
	const int page_index = m_notebook->GetSelection();
	if (page_index != wxNOT_FOUND)
		InputManager::instance().save(static_cast<size_t>(page_index));
}
		...
		if (page_data.m_controllers->GetSelection() != wxNOT_FOUND)
		{
			if (const auto data = (wxControllerData*)page_data.m_controllers->GetClientObject(page_data.m_controllers->GetSelection()))
			{
				if (const auto controller = data->ref())
				{
					...
					{
						page_data.m_controller_connected->SetBitmap(m_connected);
						InputManager::instance().save(static_cast<size_t>(i));
					}
					else
						page_data.m_controller_connected->SetBitmap(m_disconnected);
				}
...
void InputSettings2::on_timer(wxTimerEvent&)
{
	...
	if (controller) {
		...
		if (panel->consume_configuration_changed())
			save_current_player();
	}
}
...
void InputSettings2::on_profile_load(wxCommandEvent& event)
{
	...
	const bool loaded = InputManager::instance().load(page_index, selection);
	if (loaded)
	{
		InputManager::instance().save(page_index);
	}
	...
	if (loaded)
	{
		text->SetLabelText(_("Profile loaded and set as this player's active setup"));
