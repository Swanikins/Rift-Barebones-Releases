...
#include "gui/SkylanderQuickMenu.h"
...
wxPanel* GeneralSettings2::AddGeneralPage(wxNotebook* notebook)
{
	...
	{
		auto* collectionBox = new wxStaticBox(panel, wxID_ANY, _("Skylander Collection"));
		auto* collectionSizer = new wxStaticBoxSizer(collectionBox, wxVERTICAL);
		collectionSizer->Add(new wxStaticText(collectionBox, wxID_ANY,
			_("Choose the folder where your .sky, .bin, .dump, and .dmp figure files are stored.")),
			0, wxALL, 5);

		auto* pathRow = new wxBoxSizer(wxHORIZONTAL);
		auto* collectionPath = new wxTextCtrl(collectionBox, wxID_ANY,
			wxHelper::FromUtf8(GetConfig().emulated_usb_devices.skylander_collection_path.GetValue()),
			wxDefaultPosition, wxDefaultSize, wxTE_READONLY);
		collectionPath->SetMinSize(wxSize(250, -1));
		pathRow->Add(collectionPath, 1, wxALL | wxEXPAND, 5);

		auto* choosePath = new wxButton(collectionBox, wxID_ANY, _("Choose Folder"));
		choosePath->Bind(wxEVT_BUTTON, [this, collectionPath](wxCommandEvent&) {
			wxDirDialog dialog(this, _("Select Skylander collection folder"), collectionPath->GetValue(),
		...
		collectionSizer->Add(pathRow, 0, wxEXPAND, 5);
		general_panel_sizer->Add(collectionSizer, 0, wxEXPAND | wxALL, 5);
	}
