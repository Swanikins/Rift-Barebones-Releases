...
#include "gui/SkylanderManagerFrame.h"
...
enum
{
	...
	MAINFRAME_MENU_ID_TOOLS_SKYLANDER_MANAGER,
...
EVT_MENU(MAINFRAME_MENU_ID_TOOLS_SKYLANDER_MANAGER, MainWindow::OnToolsInput)
...
{
	return wxString::FromUTF8("Rift | ") + wxString::FromUTF8(BUILD_VERSION_WITH_NAME_STRING);
}
...
void MainWindow::OnToolsInput(wxCommandEvent& event)
{
	...
	case MAINFRAME_MENU_ID_TOOLS_SKYLANDER_MANAGER:
	{
		if (m_skylander_manager)
		{
			m_skylander_manager->Show(true);
			m_skylander_manager->Raise();
			m_skylander_manager->SetFocus();
		}
		else
		{
			m_skylander_manager = new SkylanderManagerFrame(this);
			m_skylander_manager->Bind(wxEVT_CLOSE_WINDOW, [this](wxCloseEvent& event) {
				if (event.CanVeto()) { m_skylander_manager->Show(false); event.Veto(); }
			});
			m_skylander_manager->Show(true);
		}
		break;
	}
	case MAINFRAME_MENU_ID_SKYLANDERS_QUICK_MENU:
		SkylanderQuickMenu_Toggle();
		break;
